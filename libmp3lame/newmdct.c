...
#include "/usr/lib/gcc-cross/arm-linux-gnueabihf/5/include/arm_neon.h"
...
{
    printf("Print some shit\n");
    printf("Two times now yall\n");
    printf("Right foot right stomp\n");
    ...
    float32x4_t v_ones = vld1q_f32(ones);
    ...
    for (ch = 0; ch < cfg->channels_out; ch++) {
        for (gr = 0; gr < cfg->mode_gr; gr++) {
            ...
            for (k = 0; k < 18 / 2; k++) {
                 ...
                 */
                // S increase
                for (band = 1; band < 32; band += 4) {
                    float32x4_t v_a = vld1q_f32( &(samp[band - 32]) );
                    float32x4_t product = vmulq_f32(v_a, v_ones);
                    vst1q_f32(&(samp[band - 32]), product);
                }
            }
